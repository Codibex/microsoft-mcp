...
public sealed class DriveTools(IGraphDriveService drive, ILogger<DriveTools> log)
{
    ...
    public Task<CallToolResult> download_file(
        ...
        CancellationToken ct = default) =>
        InvokeAsync("download_file", () => drive.DownloadAsync(itemRef, maxBytes, ct));

    [McpServerTool, Description("Create a folder (existing names get a renamed copy, no error).")]
    ...
    public Task<CallToolResult> upload_file(
        ...
        CancellationToken ct = default) =>
        InvokeAsync("upload_file", () => drive.UploadAsync(fileName, parentRef, contentText, contentBase64, ct));

    [McpServerTool, Description("Move and/or rename an item. Reversible by moving back. No delete offered.")]
    public Task<CallToolResult> move_item(
        ...
        CancellationToken ct = default) =>
        InvokeAsync("move_item", () => drive.MoveAsync(itemId, newParentRef, newName, ct));
}
