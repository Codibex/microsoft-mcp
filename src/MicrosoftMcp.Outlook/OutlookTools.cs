...
public sealed class OutlookTools(IGraphMailService mail, ILogger<OutlookTools> log)
{
    ...
    public Task<CallToolResult> search_emails(
        ...
        InvokeAsync("search_emails", () => mail.SearchAsync(new EmailQuery(query, folder, from, top), ct));
    ...
    public Task<CallToolResult> create_folder(
        ...
        CancellationToken ct = default) =>
        InvokeAsync("create_folder", () => mail.CreateFolderAsync(displayName, parentFolderId, ct));
    ...
    public Task<CallToolResult> move_email(
        [Description("Graph message id")] string messageId,
        [Description("Destination folder")] string destination,
        CancellationToken ct = default) =>
        InvokeAsync("move_email", () => mail.MoveAsync(messageId, destination, ct));

    [McpServerTool, Description("Archive a mail (move to archive folder). Reversible triage action.")]
    public Task<CallToolResult> archive_email(
        [Description("Graph message id")] string messageId,
        CancellationToken ct = default) =>
        InvokeAsync("archive_email", () => mail.ArchiveAsync(messageId, ct));

    [McpServerTool, Description("Move a mail to trash (DeletedItems). Reversible, no hard delete.")]
    public Task<CallToolResult> delete_email(
        [Description("Graph message id")] string messageId,
        CancellationToken ct = default) =>
        InvokeAsync("delete_email", async () =>
        {
    ...
    public Task<CallToolResult> create_draft(
    ...
    public Task<CallToolResult> update_draft(
        ...
        CancellationToken ct = default) =>
        InvokeAsync("update_draft", () => mail.UpdateDraftAsync(messageId, subject, body, isHtml, to, ct));

    [McpServerTool, Description("List attachment metadata (id, name, type, size) of a mail. No content download.")]
    public Task<CallToolResult> list_attachments(
        [Description("Graph message id")] string messageId,
        CancellationToken ct = default) =>
    ...
    [McpServerTool, Description("Label a mail via Outlook categories (add/remove). Use list_categories to see available ones.")]
    public Task<CallToolResult> set_categories(
        [Description("Graph message id")] string messageId,
        [Description("Categories to add")] string[] add,
        [Description("Categories to remove")] string[] remove,
        CancellationToken ct = default) =>
        InvokeAsync("set_categories", () => mail.SetCategoriesAsync(messageId, add ?? [], remove ?? [], ct));

    [McpServerTool, Description("Mark a mail read/unread.")]
    public Task<CallToolResult> mark_read(
        [Description("Graph message id")] string messageId,
        [Description("True=read, false=unread")] bool isRead,
        CancellationToken ct = default) =>
        InvokeAsync("mark_read", async () =>
        {
    ...
    public Task<CallToolResult> set_importance(
        [Description("Graph message id")] string messageId,
        [Description("low, normal or high")] string importance,
        CancellationToken ct = default) =>
        InvokeAsync("set_importance", async () =>
        {
